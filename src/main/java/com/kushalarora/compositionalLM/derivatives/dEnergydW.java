 ...
 */
public class dEnergydW extends AbstractBaseDerivativeClass implements IDerivative {
    private final dXdW dxdw;
    private INDArray dEdW;

    public dEnergydW(Model model, dXdW dxdw) {
        super(model);
        this.dxdw = dxdw;
                ...
                for (int start = 0; start < length; start++) {
                    for (int end = start + 1; end <= length; end++) {
                        for (int split = start + 1; split < end; split++) {
                            ...
                            if (udXdWShape.length != 2 ||
                                    udXdWShape[0] != 1 ||
                                    udXdWShape[1] != 1) {
                                throw new RuntimeException("udXdWArr was expected to be a matrix of shape 1 X 1");
                            }

                            float udXdW = udXdWArr.getFloat(0, 0);
                            dEdW_ij += udXdW * compositionalMu[start][end][split];
                        }
                    }
                }
